...
class TranscriptionApp:
    def __init__(self, master):
        ...
        # Dicionário para armazenar as PhotoImage, evitando que sejam coletadas pelo garbage collector
        self.icons = {} 
    ...
    def load_icon(self, name, filename):
        """Carrega um ícone e o armazena no dicionário para evitar GC."""
        try:
            # Tenta carregar o ícone. Assumimos que estão no mesmo diretório do script.
            script_dir = os.path.dirname(__file__)
            icon_path = os.path.join(script_dir, filename)
            photo_image = tk.PhotoImage(file=icon_path)
            self.icons[name] = photo_image # Armazena a referência
            return photo_image
        except tk.TclError:
            messagebox.showwarning("Erro de Ícone", f"Não foi possível carregar o ícone '{filename}'. Verifique se o arquivo existe e está no formato correto (PNG, GIF). O botão aparecerá sem ícone.")
            return None
        except FileNotFoundError:
            messagebox.showwarning("Erro de Ícone", f"O arquivo de ícone '{filename}' não foi encontrado. O botão aparecerá sem ícone.")
            return None
    ...
    def create_widgets(self):
        ...
        # Carrega o ícone da lupa
        lupa_icon = self.load_icon("lupa", "lupa.png")
        ttk.Button(audio_frame, text="Procurar", command=self.browse_audio_file, style="Small.TButton", image=lupa_icon, compound="left").grid(row=0, column=1, padx=5)
        row_idx += 1
        ...
        # Carrega o ícone do disquete e muda o texto para "Salvar"
        disquete_icon = self.load_icon("disquete", "disquete.png")
        ttk.Button(output_frame, text="Salvar", command=self.save_output_file, style="Small.TButton", image=disquete_icon, compound="left").grid(row=0, column=1, padx=5)
        ...
        # Carrega o ícone de check verde
        check_icon = self.load_icon("check_verde", "check_verde.png")
        # Botão Transcrever com ícone
        self.transcribe_button = ttk.Button(buttons_frame, text="Transcrever", command=self.start_transcription_thread, style="Green.TButton", image=check_icon, compound="left")
        self.transcribe_button.grid(row=0, column=0, padx=5, sticky="ew")

        # Carrega o ícone de stop vermelho
        stop_icon = self.load_icon("stop_vermelho", "stop_vermelho.png")
        # Botão Parar/Cancelar com ícone
        self.cancel_button = ttk.Button(buttons_frame, text="Parar/Cancelar", command=self.cancel_transcription, style="Red.TButton", image=stop_icon, compound="left")
