...
# Instala as dependências para leitura de texto (silencioso)
...
class TranscriptionApp:
    ...
    def create_widgets(self):
        ...
        # Linha 1: Opções (GPU, Análise, Solução)
        ...
        # Linha 2: Tipo de Entrada Detectado + Hyperlink Visualizar Conteúdo (à direita)
        ...
        self.visualizar_link.pack_forget()  # Escondido por padrão
        row_idx += 1

        # Linha de espaço extra antes do Modelo Whisper
        tk.Label(self.master, text="").grid(row=row_idx, column=0, columnspan=2)
        row_idx += 1

        # Linha 3: Modelo Whisper
        ...
        # Linha 4: Beam Size e Compute Type
        ...
        # Linha de espaço extra antes dos campos de entrada/saída
        tk.Label(self.master, text="").grid(row=row_idx, column=0, columnspan=2)
        row_idx += 1

        # LINHAS DE ARQUIVO DE ENTRADA E SAÍDA (ALINHAMENTO ABSOLUTO DOS BOTÕES)
        fields_frame = ttk.Frame(self.master)
        fields_frame.grid(row=row_idx, column=0, columnspan=2, padx=10, pady=5, sticky="ew")
        fields_frame.columnconfigure(1, weight=1)

        # Linha: Arquivo de Entrada
        ...
        # Linha: Salvar Saída Como (PERFEITO ALINHAMENTO COM O BOTÃO ACIMA)
        ...
        # Linha 7: Plataforma Cloud para Solução
        ...
        # --- Dois espaços de linha antes dos botões principais ---
        tk.Label(self.master, text="").grid(row=row_idx, column=0, columnspan=2, pady=1)
        row_idx += 1
        tk.Label(self.master, text="").grid(row=row_idx, column=0, columnspan=2, pady=1)
        row_idx += 1

        # Linha Botões principais
        ...
        # --- Linha do contador de tempo decorrido ---
    ...
    def save_output_file(self):
        # Garante sugestão atualizada antes de abrir o diálogo
